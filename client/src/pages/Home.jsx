...
//     // Handle donation logic here (e.g., redirect to a payment page or show a message)
    ...
    <div className="min-h-screen bg-gray-100 p-8 flex flex-col items-center">
      ...
      <div className="flex flex-col items-center w-5/6 p-10">
        {posts.map((post) => (
          <div
            key={post._id}
            className="bg-white p-6 rounded-lg shadow-md w-[60%] mb-8" // Set width to 60%
          >
            <h2 className="text-2xl font-bold mb-4 text-black">{post.title}</h2>
            <p className="text-black mb-4">{post.description}</p>
            {/* {post.imageUrl && (
              ...
              {post.imageUrl && (
                <img
                  src={`http://localhost:5000/${post.imageUrl}`}
                  alt={post.title}
                  className="w-full h-auto object-cover rounded-md mb-4" // Use h-auto instead of h-48
                />
              )}
